...
struct DetailView: View {
    ...
    private func artworkView(url: URL?, size: CGFloat) -> some View {
        let visEngine = VisualizationEngine.shared
        let ringSize = size * 1.08 // Кольца чуть больше обложки
        
        return ZStack {
            // 1. КОЛЬЦА СНИЗУ (видны по краям)
