...
struct ContentView: View {
    ...
    private func restorePlayerState() {
        ...
        if let ft = foundTrack {
            ...
            let allCUETracks = libraryVM.cueAlbums.flatMap { album in
                album.tracks.map { cue in (cue: cue, album: album) }
            }
            if let match = allCUETracks.first(where: { $0.cue.startTime == cueStart && $0.album.file.path == trackPath }) {
                let cue = match.cue
                let album = match.album
                track = Track(
                    url: URL(string: album.file.absoluteString + "#cue_\(String(format: "%.3f", cue.startTime))") ?? album.file,
