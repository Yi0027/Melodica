...
enum SettingsSection: String, CaseIterable, Identifiable {
    ...
    var titleKey: LocalizedStringKey {
        ...
        case .artwork: return "settings_artwork"
        case .statistics: return "settings_statistics"
        }
    }
    ...
    var localizedTitle: String {
        ...
        case .artwork: return NSLocalizedString("settings_artwork", comment: "")
        case .statistics: return NSLocalizedString("settings_statistics", comment: "")
        }
    }
    ...
    var searchItems: [(titleKey: String, icon: String)] {
        ...
        case .artwork:
            return [
                ("artwork_presets", "photo.on.rectangle"),
                ("add_group", "plus.rectangle")
            ]
        case .statistics:
            return [
                ("library_statistics", "chart.bar.xaxis"),
                ("top_artists", "person.2.fill"),
                ("top_albums", "opticaldisc"),
                ("top_genres", "tag.fill"),
                ("formats", "doc.on.doc.fill"),
                ("total_duration", "clock.fill")
            ]
        }
    }
}
...
//
//                    Toggle(
//                        LocalizedStringKey("enable_recommendations"),
//                        isOn: Binding(
//                            get: { settings.trackStatisticsEnabled },
//                            set: { settings.setTrackStatisticsEnabled($0) }
//                        )
//                    )
//                    .toggleStyle(.switch)
//                    .tint(settings.accent)
//
//                    Text(LocalizedStringKey("recommendations_hint"))
//                        .font(.system(size: 10))
//                        .foregroundColor(settings.textMuted.opacity(0.5))
//                }
//
...
struct ColorRow: View {
    ...
    /// Кнопка-превью. Клик открывает системный NSColorPanel.
    private var colorWellButton: some View {
        Button(action: openColorPanel) {
            RoundedRectangle(cornerRadius: 6)
                .fill(
                    Color(red: r, green: g, blue: b)
                        .opacity(alpha?.wrappedValue ?? 1.0)
                )
    ...
    private func alphaField(_ alpha: Binding<Double>) -> some View {
        VStack(spacing: 2) {
    ...
    private func openColorPanel() {
        ...
        ColorPanelManager.shared.show(
            getHex: {
                let alphaValue = aB?.wrappedValue ?? 1.0
                let color = NSColor(
                    srgbRed: rB.wrappedValue,
                    green: gB.wrappedValue,
                    blue: bB.wrappedValue,
                    alpha: alphaValue
                )
                return color.toHex()
            },
            setHex: { hex in
                ...
                rB.wrappedValue = Double(color.redComponent)
                gB.wrappedValue = Double(color.greenComponent)
                bB.wrappedValue = Double(color.blueComponent)
                if let aB = aB {
                    aB.wrappedValue = Double(color.alphaComponent)
                }
            }
        )
    }
            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if libraryVM.watchedFolders.isEmpty {
                        ...
                        VStack(spacing: 0) {
                            ForEach(Array(libraryVM.watchedFolders.enumerated()), id: \.offset) { _, folder in
                                HStack {
