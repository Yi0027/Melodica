...
struct SmartPlaylistEditorView: View {
    ...
    private func ruleRow(_ index: Int, _ editable: EditableRule) -> some View {
        HStack(spacing: 8) {
            ...
            if editable.kind == .genreContains || editable.kind == .genreEquals {
                genrePicker(selection: $rules[index].textValue)
    ...
    @ViewBuilder
    private func genrePicker(selection: Binding<String>) -> some View {
        ...
        Menu {
            ...
            HStack(spacing: 6) {
                Text(current.isEmpty
                     ? LocalizedStringKey("smart_playlist_value")
                     : LocalizedStringKey(current))
                    .foregroundColor(current.isEmpty ? .textMuted : .textMain)
                    .lineLimit(1)
                    .truncationMode(.tail)
                Spacer(minLength: 0)
...
struct EditableRule: Identifiable {
    ...
    init(rule: SmartPlaylistRule) {
        ...
        case .hasRating(let v):
            kind = .hasRating
            boolValue = v
        case .ratingGreaterThan(let v):
            kind = .ratingGreaterThan
            numberValue = String(v)

        case .ratingLessThan(let v):
            kind = .ratingLessThan
            numberValue = String(v)

        case .ratingEquals(let v):
            kind = .ratingEquals
            numberValue = String(v)
        }
    }
    ...
    func toRule() -> SmartPlaylistRule? {
        ...
        case .hasRating:
            return .hasRating(boolValue)
        case .ratingGreaterThan:
            guard let v = Int(numberValue) else { return nil }
            return .ratingGreaterThan(v)

        case .ratingLessThan:
            guard let v = Int(numberValue) else { return nil }
            return .ratingLessThan(v)

        case .ratingEquals:
            guard let v = Int(numberValue) else { return nil }
            return .ratingEquals(v)
        }
    }
}
