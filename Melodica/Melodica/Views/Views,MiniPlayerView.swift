...
// MARK: - Progress section isolation

enum MiniPlayerProgressStyle {
    case dark   // MiniPlayerView — белые элементы на тёмном фоне
    case light  // LiquidGlassMiniPlayerView — акцентный цвет
}

struct MiniPlayerProgressSection: View {
    @ObservedObject var playerVM: PlayerViewModel
    @ObservedObject var visEngine: VisualizationEngine
    @ObservedObject var progress: PlaybackProgress
    let style: MiniPlayerProgressStyle
    ...
    var body: some View {
        if visEngine.mode == .waveform && visEngine.isRunning {
            WaveformProgressView(
                ...
                accentColor: style == .dark ? .white : .accentColor
            )
        } else {
            ProgressBarView(
                progress: fraction,
