...
struct LRCSettingsView: View {
    ...
    private var tagsContent: some View {
        ...
        VStack(spacing: 6) {
            // Пословная подсветка — только в general
            if selectedTab == "general" {
                HStack(spacing: 10) {
                    Toggle("", isOn: Binding(
                        get: { settings.lrcWordHighlight },
                        set: { settings.setLRCWordHighlight($0) }
                    ))
                    .toggleStyle(.checkbox)
                    .labelsHidden()
                    .scaleEffect(0.8)
                    ...
                    Text(NSLocalizedString("lrc_word_highlight", comment: ""))
                        .font(.system(size: 12, weight: .medium))
                        .foregroundColor(settings.textMain)
                ...
                LRCTagRow(
                    ...
                    isEnabled: Binding(
                        ...
                        set: { newValue in DispatchQueue.main.async {
                            ...
                            // Не занимают место в лимите: note и inst.
                            // Всё остальное считается.
                            let nonCountingTags: Set<String> = ["note", "inst"]
                            if !settings.lrcUnlimitedFields && newValue && n >= 5
                                && !nonCountingTags.contains(tagInfo.tag)
                                && !(tags[tagInfo.tag] ?? false) { return }
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(label)
                    .font(.system(size: 12, weight: .medium))
                    .foregroundColor(isEnabled ? settings.textMain : settings.textMuted.opacity(0.4))
                Text("<\(tag)>")
                    .font(.system(size: 9, design: .monospaced))
                    .foregroundColor(settings.textMuted.opacity(0.4))
            }
            ...
            Text("Aa")
                ...
                .foregroundColor(isEnabled ? Color(nsColor: NSColor(hex: color) ?? .white) : Color.clear)
                .frame(width: 28)
        }
        .padding(.horizontal, 10)
        .padding(.vertical, 6)
...
class ColorPanelManager: NSObject {
    ...
    private var getHex: (() -> String)?
    private var setHex: ((String) -> Void)?

    /// Удобный API для тех, у кого hex-строка.
    func show(hex: Binding<String>) {
        show(
            getHex: { hex.wrappedValue },
            setHex: { hex.wrappedValue = $0 }
        )
    }

    /// Универсальный API — для цветов, хранимых как (r, g, b, a).
    func show(
        getHex: @escaping () -> String,
        ...
        panel.color = NSColor(hex: getHex()) ?? .white
