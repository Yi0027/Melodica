...
final class LibraryViewModel: ObservableObject {
    ...
    private func scanFolder(_ folderURL: URL) {
        ...
        guard let enumerator = fm.enumerator(at: folderURL, includingPropertiesForKeys: [.isRegularFileKey], options: [.skipsHiddenFiles, .skipsPackageDescendants]) else {
            isLoading = false
            isScanning = false
            processQueue()
            return
        ...
        for case let url as URL in enumerator {
            if supportedExtensions.contains(url.pathExtension.lowercased()) { allURLs.append(url) }
        }
        let urls = allURLs
        ...
        Task.detached(priority: .high) {
            ...
            await MainActor.run {
                ...
                // Обрабатываем очередь, ТОЛЬКО если нет обогащения
                if !self.isEnriching {
                    self.startBackgroundEnrichment()
                } else {
                    // Если обогащение уже идёт — папка добавится после его завершения
                    // (processQueue вызывается в конце startBackgroundEnrichment)
                }
    ...
    func startBackgroundEnrichment() {
        ...
        let tracksToEnrich = tracks.filter { $0.albumArtURL == nil || $0.replayGain == nil }
        
        guard !tracksToEnrich.isEmpty else {
            isEnriching = false
            isLoading = false
            stopEnrichTimer()
            processQueue()
        ...
        let total = tracksToEnrich.count
        
        Task.detached(priority: .background) {
            // === Фаза 1: RG, LRC, albumArtist ===
            // === Фаза 1: RG, LRC, albumArtist ===
            await MainActor.run {
                self.enrichPhase = 1
            ...
            for (index, track) in tracksToEnrich.enumerated() {
                ...
                let enriched = await MetadataReader.enrichTrack(track)
                await MainActor.run {
                    if let idx = self.tracks.firstIndex(where: { $0.url.path == track.url.path }) {
                        self.tracks[idx] = enriched
                    }
                    self.enrichProgress = (index + 1, total)
                    if index % 10 == 0 { self.updateFilteredTracks() }
                }
            }
            ...
            // === Фаза 2: Обложки в полном размере ===
            await MainActor.run {
                self.enrichPhase = 2
                self.enrichProgress = (0, total)
                self.enrichLastRemaining = 0
            ...
            for (index, track) in tracksToEnrich.enumerated() {
                ...
                if let artURL = await MetadataReader.enrichArtwork(for: track) {
                    await MainActor.run {
                        if let idx = self.tracks.firstIndex(where: { $0.url.path == track.url.path }) {
                            self.tracks[idx].albumArtURL = artURL
                        }
                        self.enrichProgress = (index + 1, total)
                        if index % 10 == 0 { self.updateFilteredTracks() }
                    }
            ...
            // === Фаза 3: Миниатюры (только если это последняя папка в очереди) ===
            let shouldCreateThumbnails = await MainActor.run { self.folderQueue.isEmpty }
            
            if shouldCreateThumbnails {
                let tracksForThumbnails = await MainActor.run { self.tracks.filter { $0.albumArtURL != nil } }
                let thumbTotal = tracksForThumbnails.count
                
                if thumbTotal > 0 {
                    await MainActor.run {
                        self.enrichPhase = 3
                        self.enrichProgress = (0, max(thumbTotal, 1))
                        self.enrichLastRemaining = 0
                        self.startEnrichTimer()
                    }
                    ...
                    for (index, track) in tracksForThumbnails.enumerated() {
                        ...
                        await MainActor.run {
                            self.enrichProgress = (index + 1, max(thumbTotal, 1))
                        }
            ...
            await MainActor.run {
                ...
                // Обновляем треки в плейлистах из обогащённых
                if let libState = PlayerStateManager.shared.loadLibrary() {
                    for cached in libState.cachedPlaylists {
                        if let idx = self.playlists.firstIndex(where: { $0.name == cached.name }) {
                            let refreshed: [Track] = cached.trackPaths.compactMap { path in
                                if let match = self.tracks.first(where: { $0.url.path == path }) {
                                    return match
                                }
                                if let match = self.tracks.first(where: { $0.url.lastPathComponent == URL(fileURLWithPath: path).lastPathComponent }) {
                                    return match
                                }
                                return nil  // Не создаём заглушку
                            }
                            self.playlists[idx] = (cached.name, refreshed)
                        } else {
                            let tracks: [Track] = cached.trackPaths.compactMap { path in
                                if let match = self.tracks.first(where: { $0.url.path == path }) {
                                    return match
                                }
                                if let match = self.tracks.first(where: { $0.url.lastPathComponent == URL(fileURLWithPath: path).lastPathComponent }) {
                            ...
                            self.playlists.append((cached.name, tracks))
                        }
                    }
                }
    ...
    private func savePlaylistsToCache() {
        var state = PlayerStateManager.shared.loadLibrary() ?? LibraryState(
            lastFolderPath: nil, playlistPaths: [], savedPlaylists: []
        )
