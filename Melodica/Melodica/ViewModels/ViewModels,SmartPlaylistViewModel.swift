...
final class SmartPlaylistViewModel: ObservableObject {
    ...
    func refreshCache(
        ...
        trackCache[playlist.id] = computed.map { $0.smartHash ?? $0.url.path }
        saveCache()
    }
    ...
    private func resolveTracks(
        ...
        for path in paths {
            if let track = library.first(where: { $0.smartHash == path }) {
                resolved.append(track)
            } else if let track = library.first(where: { $0.url.path == path }) {
                resolved.append(track)
