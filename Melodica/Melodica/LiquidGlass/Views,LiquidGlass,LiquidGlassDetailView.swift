...
struct LiquidGlassDetailView: View {
            ...
            VStack(spacing: 6) {
                ...
                if let track = playerVM.currentTrack {
                    ZStack {
                        ...
                        artworkImage(for: track.albumArtURL)
            ...
            if let track = playerVM.currentTrack {
                if !lyrics.isEmpty {
                    LiquidGlassLyricsView(
                        ...
                        trackId: track.id,
                        onTapLine: { time in
                            let fraction = time / (playerVM.progress.duration > 0 ? playerVM.progress.duration : 1)
    ...
    @ViewBuilder
    private func artworkImage(for url: URL?) -> some View {
        ZStack {
            ...
            if previousArtwork == nil && currentArtwork == nil {
        ...
        .task(id: url) {
            await loadArtwork(url)
        }
    }

    private func loadArtwork(_ url: URL?) async {
        // Старую — в previous, чтобы держалась пока грузится новая.
        previousArtwork = currentArtwork
        currentArtwork = nil

        guard let url else {
            // Обложки нет — просто уводим старую через fade.
        ...
        // Проверяем отмену перед тяжёлой загрузкой.
        if Task.isCancelled { return }

        guard let nsImage = NSImage(contentsOf: url) else { return }

        // Ещё одна проверка после загрузки — если URL сменился,
        // task(id:) уже отменил эту задачу, не рисуем устаревшее.
        if Task.isCancelled { return }
