// Views/LiquidGlassEQPresetSection.swift
import SwiftUI

// MARK: - Preset Section (Liquid Glass)

struct LiquidGlassEQPresetSection: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingCreate) {
            LiquidGlassEQPresetEditorSheet(existingPreset: nil)
        }
        .sheet(item: $editingPreset) { preset in
            LiquidGlassEQPresetEditorSheet(existingPreset: preset)
        }
    }
    ...
    private func presetRow(_ preset: EQPreset) -> some View {
            ...
            Button {
                ...
                HStack(spacing: 8) {
                    Image(systemName: isActive ? "checkmark.circle.fill" : "circle")
                        .font(.system(size: 12))
                        .foregroundColor(isActive ? systemAccent : Color.secondary.opacity(0.4))

                    Text(preset.name)
                        .font(.system(size: 12))
                        .foregroundColor(isActive ? systemAccent : .primary)
                        .lineLimit(1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 6)
                .fill(isActive ? systemAccent.opacity(0.12) : .clear)
        )
    }
}

// MARK: - Editor sheet (Liquid Glass)

private struct LiquidGlassEQPresetEditorSheet: View {
    ...
    /// Жанры, доступные для выбора:
    /// — скрыты те, что уже закреплены за другими пресетами;
    /// — опционально отфильтрованы по строке поиска.
    ...
    /// Сколько жанров скрыто из-за того, что заняты другими пресетами.
        ...
        return Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: isSelected ? "checkmark.square.fill" : "square")
                    .font(.system(size: 12))
                    .foregroundColor(isSelected ? systemAccent : Color.secondary.opacity(0.5))
            ...
            .background(
                RoundedRectangle(cornerRadius: 4)
                    .fill(isSelected ? systemAccent.opacity(0.12) : .clear)
            )
        }
        .buttonStyle(.plain)
    }
        ...
        HStack(spacing: 8) {
            ...
            Button {
                ...
                Text(LocalizedStringKey("save"))
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6)
                            .fill(canSave ? systemAccent : systemAccent.opacity(0.4))
                    )
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
