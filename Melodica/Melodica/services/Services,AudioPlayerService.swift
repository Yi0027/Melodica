...
final class AudioPlayerService: NSObject, ObservableObject {
    ...
    private func recreateEngine() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak self] in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                if currentTime > 0 { self.seek(to: currentTime) }
    ...
    func play(_ track: Track, volume: Float? = nil, startTime: TimeInterval? = nil, duration: TimeInterval? = nil, completion: @escaping () -> Void) {
        stopCrossfade()
        stopDisplayLink()
        
        // ✅ Отмена старой генерации и сброс волны
        ...
        // Извлекаем реальный URL для CUE треков
        let realURL: URL = {
        ...
        // Всегда генерируем волну для кеша, если включён passive/active режим
        ...
        var slot = PlayerSlot(playerNode: playerNode, eqNode: eqNode)
        slots.append(slot)
        activeSlot = 0
        ...
        do {
            ...
            engine = newEngine
            startDisplayLink()
            installMainMixerTap()  // ✅ Добавьте эту строку
    ...
    private func installMainMixerTap() {
        guard let engine = engine else { return }
        
        // Удаляем старые tap'ы
        engine.mainMixerNode.removeTap(onBus: 0)
        
        let needVisualization = VisualizationEngine.shared.mode != .off
        let needSilenceDetection = settings.silenceSkipEnabled
        
        guard needVisualization || needSilenceDetection else {
            return
        }
        
        // ✅ Для визуализации берем сигнал с EQ ноды (до применения громкости)
        if needVisualization {
            let eqNode = slots[activeSlot].eqNode
            let format = eqNode.outputFormat(forBus: 0)
            
            eqNode.removeTap(onBus: 0)
            eqNode.installTap(onBus: 0, bufferSize: 4096, format: format) { [weak self] buffer, _ in
        ...
        // ✅ Для детекции тишины используем main mixer (учитывает громкость)
        if needSilenceDetection {
    ...
    // Метод для переустановки tap при изменении настроек
    ...
    func seek(to time: TimeInterval) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in self?.isSeeking = false }
    }
    ...
    func preloadNextTrack(_ track: Track, volume: Float? = nil, completion: @escaping () -> Void) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            guard let audioFile = try? AVAudioFile(forReading: realURL) else { return }
    ...
    private func resumeCrossfade() {
        ...
        guard let newIndex = preloadedSlot else { return }
        let oldIndex = activeSlot
    ...
    private func startDisplayLink() {
        ...
        timer.setEventHandler { [weak self] in
            ...
            if self.switchingTrack {
                ...
                guard let startDate = crossfadeStartDate else { return }
                ...
                if slots[self.activeSlot].track?.cueStartTime != nil {
                    currentTimeValue = (slots[self.activeSlot].cueSeekTime ?? 0) + segmentTime
                } else {
                    currentTimeValue = self.seekOffset + segmentTime
                }
            }
            
            DispatchQueue.main.async { [weak self] in self?.currentTime = currentTimeValue }
            
            if !self.crossfadeStarted && !self.isSeeking && self.isPlaying {
                let slot = self.slots[self.activeSlot]
                let maxTime: Double
                if let track = slot.track, track.cueStartTime != nil, track.duration > 0 {
                    maxTime = track.duration
                } else {
                    maxTime = Double(slot.fileLength) / slot.sampleRate
                }
    ...
    private func startSilenceDetection() {
        // Теперь используем общий метод установки tap
        installMainMixerTap()
    }
