...
enum MetadataReader {
    ...
    static func readTrack(from url: URL) async -> Track? {
        ...
        do {
            ...
            for item in metadata {
                ...
                case "itsk/com.apple.iTunes.rating", "itsk/rating", "itsk/RATING":
                    if rating == nil, let val = Int(value) {
                        rating = val > 100 ? val : (val * 255 / 100)
                    }
        ...
        if replayGain == nil || replayGainAlbum == nil,
           url.pathExtension.lowercased() == "mp3" {
            ...
            // Если AVAsset всё равно не дал — читаем ID3 вручную
            if replayGain == nil || replayGainAlbum == nil {
    ...
    static func createThumbnails(for artURL: URL?, sourceURL: URL) async {
        guard let artURL, let image = NSImage(contentsOf: artURL) else { return }
