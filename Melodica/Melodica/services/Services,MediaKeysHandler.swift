...
class MediaKeysHandler {
    ...
    private func setupRemoteCommands() {
        ...
        // Play — НЕ toggle. Система может прислать play автоматически
        // (BT handshake, wake, смена аудиоустройства), и toggle из паузы
        // даст неверный результат.
        center.playCommand.isEnabled = true
        center.playCommand.addTarget { [weak self] _ in
            DispatchQueue.main.async {
                self?.playerVM?.playFromRemote()
                self?.updateNowPlayingInfo()
            }
            return .success
        }
        
        // Pause — НЕ toggle. Симметрично play.
        center.pauseCommand.isEnabled = true
        center.pauseCommand.addTarget { [weak self] _ in
            DispatchQueue.main.async {
                self?.playerVM?.pauseFromRemote()
                self?.updateNowPlayingInfo()
            }
            return .success
        }
        
        // Toggle Play/Pause — здесь toggle оставляем, это его семантика.
        ...
        center.changePlaybackPositionCommand.addTarget { [weak self] event in
            ...
            DispatchQueue.main.async {
                guard let playerVM = self?.playerVM, playerVM.duration > 0 else {
                    return
                }
